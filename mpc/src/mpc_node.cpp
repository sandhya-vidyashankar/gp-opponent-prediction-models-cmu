...
public:
    MPC() : Node("mpc_node")
    {
        ...
        this->config_ = Config("/sim_ws/src/gp-opponent-prediction-models-cmu/mpc/config/config.yaml");
        ...
        this->race_line.getWaypoints(config_, nullptr);
        RCLCPP_INFO(this->get_logger(), "Loaded waypoints: %i", this->race_line.x.size());
    ...
    void pose_callback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg)
    {
        ...
        if(this->race_line.x.empty()){
            ...
            //RCLCPP_INFO(this->get_logger(), "Xref: %i %i", this->mpc_solver.Xref.size1());
            control = this->mpc_solver.solve(this->car_state_, this->steer, this->acc);
            this->acc = get<1>(control);
            this->steer = get<0>(control);
            this->vel += acc*config_.dt;
            RCLCPP_INFO(this->get_logger(), "acc %f, steer %f, vel %f", this->acc, this->steer, this->vel);
        }
        ackermann_msgs::msg::AckermannDriveStamped drive_msg;
        drive_msg.header.stamp = this->now();
        drive_msg.header.frame_id = "map";
        drive_msg.drive.steering_angle = this->steer;
        drive_msg.drive.acceleration = this->acc;
    ...
    float get_robot_vel(nav_msgs::msg::Odometry robot_odom){
        this->actual_vel = std::sqrt(robot_odom.twist.twist.linear.x *robot_odom.twist.twist.linear.x + robot_odom.twist.twist.linear.y * robot_odom.twist.twist.linear.y);
        return std::max(this->actual_vel, 1.0f);
    }
    void path_publish(){
    ...
    void rrt_callback(const nav_msgs::msg::Path::ConstSharedPtr rrt_path_msg){
        this->path_updated = false;
        // RCLCPP_INFO(this->get_logger(), "RRT path callback");
        // this->race_line.getWaypoints(this->config_, rrt_path_msg);
        this->path_updated = true;
    }
